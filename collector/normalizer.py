...
def normalize(raw_article, source):
    ...
    return {
        ...
        # Never fall through to "" — the DB has unique (source_name, external_id),
        # and Postgres treats two empty strings as equal (unlike two NULLs), so a
        # blank external_id would collide across articles and silently vanish.
        "external_id": raw_article.get("external_id") or raw_article.get("source_url") or None,
